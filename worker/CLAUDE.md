...
- A TV-client OAuth token is accepted only with a `TVHTML5` context on www.youtube.com; `WEB_REMIX`/`WEB`/`ANDROID*` answer `400 INVALID_ARGUMENT`, and `TVHTML5` cannot create playlists (`Precondition check failed`) → Data API `playlists.insert` (D3). Anonymous search from the Worker: `ANDROID_MUSIC` passes where `WEB_REMIX` is blocked; fixtures `android-search-*.json` were recorded from the edge with a scratch Worker under `wrangler dev --remote` (pruned of thumbnails, menus and player configs). Searches are anonymous, so throttling is per egress IP (D8): the abuse page (403 HTML "Sorry…") is a throttle, handled by backoff + the KV cache.
