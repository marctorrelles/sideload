...
const captureAdds = (opts: { body?: unknown }) => {
  added.push(...JSON.parse(String(opts.body)).actions.map((a: { addedVideoId: string }) => a.addedVideoId));
  return { status: 'STATUS_SUCCEEDED' };
};
const entry = (id: string, name: string, extra: object = {}) => ({
...
const payload = (id: string) => ({
  id,
  spotify: { clientId: 'c'.repeat(32), access: 'SA', refresh: 'SR', expiresAt: Date.now() + 3600_000 },
  google: { access: 'GA', refresh: 'GR', expiresAt: Date.now() + 3600_000 },
  selection: {
    liked: false,
    likedCount: 0,
    playlists: [{ id: 'P'.repeat(22), name: 'Deep Focus', description: null, isPublic: false, trackCount: 3 }],
    albums: [],
    artists: [],
  },
});
...
const until = (stub: Stub, pred: (v: JobView) => boolean) =>
  vi.waitFor(
    async () => {
      const v = await stub.view();
      if (!v || !pred(v)) throw new Error(`not yet: ${v?.status}`);
    },
...
describe('JobDO', () => {
  ...
  beforeEach(async () => {
    await reset();
    stub = env.JOB.get(env.JOB.idFromName(id + Math.random()));
    added = [];
  }); // reset(): KV is shared across tests in this pool; the match cache must not leak between cases
  ...
  it('moves a playlist end to end, lists review items, wipes spotify tokens on finish, yt tokens after 24 h, everything after 7 days', async () => {
    sp()
      .intercept({ path: (p) => p.startsWith(`/v1/playlists/${'P'.repeat(22)}/items`) })
      .reply(200, {
        total: 3,
        next: null,
        items: [
          entry('t1', 'Xtal'),
          entry('t2', 'Nothing Will Match This Zzz'),
          {
            added_at: null,
            is_local: true,
            item: {
    ...
    music()
      .intercept({ ...SEARCH, body: q('Xtal') })
      .reply(200, songs);
    music()
      .intercept({ ...SEARCH, body: q('Zzz') })
      .reply(200, { contents: {} })
      .times(2); // songs, then the videos fallback → nothing
    data().intercept(CREATE).reply(200, { id: 'PLnew' });
    tv().intercept(EDIT).reply(200, captureAdds);
    data()
      .intercept(READBACK)
      .reply(200, () => pageOf(added)); // verify pass sees everything → done
    ...
    expect(v.recent.map((e) => e.kind)).toEqual(
      expect.arrayContaining(['read', 'match', 'review', 'create', 'add', 'verify']),
    ); // the live feed
    ...
    await runInDurableObject(stub, (_, state) => {
      const row = state.storage.sql.exec('SELECT spotify_tokens, yt_tokens FROM job').one();
      expect(row.spotify_tokens).toBeNull();
      expect(row.yt_tokens).not.toBeNull();
    });
    ...
    await runInDurableObject(stub, (_, state) => {
      state.storage.sql.exec('UPDATE job SET finished_at = ?', Date.now() - 25 * 3600_000);
    });
    expect(await runDurableObjectAlarm(stub)).toBe(true);
    await runInDurableObject(stub, (_, state) => {
      expect(state.storage.sql.exec('SELECT yt_tokens FROM job').one().yt_tokens).toBeNull();
    });
    expect((await stub.view())!.ytConnected).toBe(false);
    await runInDurableObject(stub, (_, state) => {
      state.storage.sql.exec('UPDATE job SET finished_at = ?', Date.now() - 8 * 24 * 3600_000);
    });
    expect(await runDurableObjectAlarm(stub)).toBe(true);
  ...
  it('backs off on throttling and resumes where it stopped', async () => {
    sp()
      .intercept({ path: (p) => p.startsWith('/v1/playlists/') })
      .reply(200, { total: 1, next: null, items: [entry('t1', 'Xtal')] });
    music()
      .intercept({ ...SEARCH, body: q('Xtal') })
      .reply(200, '<html>throttled</html>');
    await stub.start(payload(id));
  ...
  it('pause stops work; resume continues; resolve writes; disconnect wipes yt token', async () => {
    ...
    sp()
      .intercept({ path: (p) => p.startsWith('/v1/playlists/') })
      .reply(200, { total: 1, next: null, items: [entry('t1', 'Nothing Will Match This Zzz')] });
    music()
      .intercept({ ...SEARCH, body: q('Zzz') })
      .reply(200, { contents: {} })
      .times(2);
    ...
    const v = (await stub.view())!;
    expect(v.status).toBe('done');
    expect(v.review.length).toBe(1);
    tv()
      .intercept({ ...EDIT, body: (b) => JSON.parse(String(b)).actions[0].addedVideoId === 'abcdefghijk' })
      .reply(200, { status: 'STATUS_SUCCEEDED' });
    ...
    await runInDurableObject(stub, (_, state) => {
      state.storage.sql.exec("UPDATE item SET status = 'matching'");
      state.storage.sql.exec("UPDATE track SET status = 'review' WHERE status = 'moved'");
    });
  ...
  it('reads back after writing and re-drives a silent no-op until it converges (D15)', async () => {
    ...
    data()
      .intercept(READBACK)
      .reply(200, () => pageOf(added)); // after the re-drive it is there
    await stub.start(payload(id));
    await settle(stub);
    const v = (await stub.view())!;
    expect(v.status).toBe('done');
    expect(v.totals.moved).toBe(1);
    expect(v.review.length).toBe(0);
    expect(added.length).toBe(2); // written twice
    await runInDurableObject(stub, (_, state) => {
      expect(
        (state.storage.sql.exec('SELECT verify_passes FROM item').one() as { verify_passes: number }).verify_passes,
      ).toBe(2);
    });
  });
  ...
  it('sends a collapsed match to review instead of silently merging it (D16)', async () => {
    ...
    sp()
      ...
      .reply(200, {
        total: 2,
        next: null,
        items: [entry('t1', 'Xtal'), entry('t2', 'Xtal', { album: { name: 'Some Compilation' } })],
      });
    music()
      .intercept({ ...SEARCH, body: q('Xtal') })
      .reply(200, songs)
      .times(2); // both miss the cache at the same moment (parallel) and land on the same best video
    data().intercept(CREATE).reply(200, { id: 'PLnew' });
    tv().intercept(EDIT).reply(200, captureAdds);
    data()
      .intercept(READBACK)
  ...
  it('fails cleanly on expired YouTube auth and wipes both tokens', async () => {
    ...
    data()
      .intercept(CREATE)
      .reply(401, { error: { code: 401, message: 'Invalid Credentials' } }); // token revoked: the first authenticated call fails
    await stub.start(payload(id));
    await settle(stub);
    const v = (await stub.view())!;
    expect(v.status).toBe('failed');
    expect(v.failure).toBe('auth_expired');
    expect(v.ytConnected).toBe(false);
  });
  ...
  it('fails just the item when Spotify refuses a playlist (403 on playlists owned by others in Development Mode)', async () => {
    ...
    sp()
      ...
      .reply(200, { total: 1, next: null, items: [entry('t1', 'Xtal')] });
    sp()
      .intercept({ path: (p) => p.startsWith(`/v1/playlists/${other}/items`) })
    ...
    data()
      ...
      .reply(200, () => pageOf(added));
    const p = payload(id);
    p.selection.playlists.push({
      id: other,
      name: 'Somebody else’s mix',
      description: null,
      isPublic: true,
      trackCount: 75,
    });
    ...
    expect(v.review.find((r) => r.reason === 'not_accessible')).toMatchObject({
      title: 'Somebody else’s mix',
      itemName: 'Playlists',
      actionable: false,
    });
  });
  ...
  it('reads the built-in demo library when the job carries demo tokens', async () => {
    const p = payload(id);
    p.spotify = demoTokens('deadbeef'.repeat(4));
    p.selection.playlists = [
      { id: 'demolatenights00000000', name: 'Late nights', description: null, isPublic: false, trackCount: 5 },
    ];
  ...
  it('rejects oversize selections up front', async () => {
    expect(
      await stub.start({
        ...payload(id),
        selection: { liked: true, likedCount: 30000, playlists: [], albums: [], artists: [] },
      }),
