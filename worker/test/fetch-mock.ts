...
export interface InterceptOptions {
  path: Matcher;
  method?: Matcher;
  headers?: Record<string, Matcher> | ((h: Record<string, string>) => boolean);
  body?: Matcher;
}
...
const match = (m: Matcher | undefined, v: string): boolean =>
  m === undefined || (typeof m === 'string' ? m === v : m instanceof RegExp ? m.test(v) : m(v));
...
class Interceptable {
  ...
  intercept(opts: InterceptOptions) {
    return new MockInterceptor(this.origin, opts, this.list);
  }
}
...
class FetchMock {
  ...
  get(origin: string) {
    return new Interceptable(origin, this.list);
  }
  pendingInterceptors() {
    return this.list.filter((i) => i.times > 0 && i.times !== Infinity);
  }
  assertNoPendingInterceptors() {
    const p = this.pendingInterceptors();
    if (p.length)
      throw new Error(
        `${p.length} interceptor(s) not consumed: ${p.map((i) => `${String(i.opts.method ?? 'GET')} ${i.origin}${typeof i.opts.path === 'string' ? i.opts.path : '<fn>'}`).join(', ')}`,
      );
  }
  private async dispatch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
    ...
    const body =
      req.method === 'GET' || req.method === 'HEAD' ? undefined : new TextDecoder().decode(await req.arrayBuffer()); // arrayBuffer: workerd warns on .text() for form bodies
    const i = this.list.find(
      (i) =>
        i.times > 0 &&
        i.origin === url.origin &&
        match(i.opts.path, path) &&
        match(i.opts.method ?? 'GET', req.method) &&
        (!i.opts.headers ||
          (typeof i.opts.headers === 'function'
            ? i.opts.headers(headers)
            : Object.entries(i.opts.headers).every(([k, m]) => match(m, headers[k.toLowerCase()] ?? '')))) &&
    ...
    if (i.delayMs)
      await new Promise<void>((res, rej) => {
        ...
        req.signal.addEventListener('abort', () => {
          clearTimeout(t);
          rej(req.signal.reason ?? new DOMException('aborted', 'AbortError'));
        });
      });
    const data =
      typeof i.data === 'function' ? i.data({ path, origin: url.origin, method: req.method, body, headers }) : i.data;
