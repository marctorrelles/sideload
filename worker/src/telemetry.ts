...
export async function track(env: Pick<Env, 'MIXPANEL_TOKEN' | 'MIXPANEL_API'>, event: string, distinctId: string | undefined, props: Props = {}): Promise<void> {
  if (!env.MIXPANEL_TOKEN) return;
  const body = [{ event, properties: { token: env.MIXPANEL_TOKEN, distinct_id: distinctId ?? 'anonymous', time: Date.now(), $insert_id: crypto.randomUUID(), ...props } }];
  try {
    const r = await fetch(`${env.MIXPANEL_API ?? 'https://api.mixpanel.com'}/track?ip=0`, { method: 'POST', headers: { 'content-type': 'application/json', accept: 'text/plain' }, body: JSON.stringify(body), signal: AbortSignal.timeout(5_000) });
