...
const P = { songs: 'EgWKAQIIAWoMEA4QChADEAQQCRAF', albums: 'EgWKAQIYAWoMEA4QChADEAQQCRAF', artists: 'EgWKAQIgAWoMEA4QChADEAQQCRAF' };
...
async function getToken(): Promise<string> {
  if (existsSync(TOKFILE)) {
    ...
    if (r.access_token) { writeFileSync(TOKFILE, JSON.stringify({ access: r.access_token, refresh: t.refresh, expiresAt: Date.now() + r.expires_in * 1000 })); return r.access_token; }
  }
  ...
  for (;;) {
    ...
    const t = await form('https://oauth2.googleapis.com/token', { client_id: env.GOOGLE_CLIENT_ID!, client_secret: env.GOOGLE_CLIENT_SECRET!, device_code: dc.device_code, grant_type: 'urn:ietf:params:oauth:grant-type:device_code' });
    if (t.access_token) { writeFileSync(TOKFILE, JSON.stringify({ access: t.access_token, refresh: t.refresh_token, expiresAt: Date.now() + t.expires_in * 1000 })); return t.access_token; }
...
async function post(label: string, url: string, headers: Record<string, string>, body: object, retry = true): Promise<any> {
  ...
  if (r.status === 403 && text.includes('<title>Sorry')) { console.log(`${label} → abuse page${retry ? '; waiting 45 s, retrying once' : ''}`); if (!retry) return null; await pause(45_000); return post(label, url, headers, body, false); }
  let j: any = null; try { j = JSON.parse(text); } catch {}
  console.log(`${label} → ${r.status}${j?.error ? ' ❌ ' + String(j.error.message ?? '').slice(0, 100) : j ? ' ✅' : ' ❌ ' + text.slice(0, 80)}`);
  return j;
}
const d = new Date().toISOString().slice(0, 10).replace(/-/g, '');
const anon = (endpoint: string, body: object) => post(`ANON ${endpoint}`, `https://music.youtube.com/youtubei/v1/${endpoint}?prettyPrint=false`, { 'content-type': 'application/json', 'user-agent': UA, origin: 'https://music.youtube.com', 'x-origin': 'https://music.youtube.com' }, { ...body, context: { client: { clientName: 'WEB_REMIX', clientVersion: `1.${d}.01.00`, hl: 'en', gl: 'US' }, user: {} } });
const tv = (token: string, endpoint: string, body: object) => post(`TV ${endpoint}`, `https://www.youtube.com/youtubei/v1/${endpoint}?prettyPrint=false`, { 'content-type': 'application/json', 'user-agent': TV_UA, authorization: `Bearer ${token}` }, { ...body, context: { client: { clientName: 'TVHTML5', clientVersion: '7.20250120.19.00', hl: 'en', gl: 'US' }, user: {} } });
...
(async () => {
  ...
  const s = await anon('search', { query: 'Aphex Twin Xtal', params: P.songs }); if (s) save('innertube-search-songs', s);
  const videoId = JSON.stringify(s ?? '').match(/"videoId":"([\w-]{11})"/)?.[1]; console.log('  videoId', videoId);
  await pause();
  const a = await anon('search', { query: 'Aphex Twin Selected Ambient Works 85-92', params: P.albums }); if (a) save('innertube-search-albums', a);
  const albumBrowseId = JSON.stringify(a ?? '').match(/"browseId":"(MPRE[\w-]+)"/)?.[1]; await pause();
  const ar = await anon('search', { query: 'Aphex Twin', params: P.artists }); if (ar) save('innertube-search-artists', ar);
  const channelId = JSON.stringify(ar ?? '').match(/"browseId":"(UC[\w-]{22})"/)?.[1]; await pause();
  let albumPl: string | undefined;
  if (albumBrowseId) { const b = await anon('browse', { browseId: albumBrowseId }); if (b) save('innertube-browse-album', { microformat: b.microformat, contents: {} }, '; trimmed to microformat'); albumPl = String(b?.microformat?.microformatDataRenderer?.urlCanonical ?? '').match(/list=([\w-]+)/)?.[1]; await pause(); }
  if (!videoId) throw new Error('anonymous search failed twice; wait a few minutes and rerun');
  // 2. create (Data API) + writes (TV)
  const c = await dataApi(token, 'POST', 'playlists?part=snippet,status', { snippet: { title: 'sideload spike (delete me)' }, status: { privacyStatus: 'private' } }); save('data-playlist-insert', c);
  const pl: string = c.id; await pause();
  save('innertube-edit-playlist', await tv(token, 'browse/edit_playlist', { playlistId: pl, actions: [{ action: 'ACTION_ADD_VIDEO', addedVideoId: videoId }] })); await pause();
  save('innertube-like', await tv(token, 'like/like', { target: { videoId } })); await pause();
  if (albumPl) { await tv(token, 'like/like', { target: { playlistId: albumPl } }); await pause(); }
  if (channelId) { save('innertube-subscribe', await tv(token, 'subscription/subscribe', { channelIds: [channelId] })); await pause(5000); }
  // 3. read-backs (Data API)
  const di = await dataApi(token, 'GET', `playlistItems?part=contentDetails&maxResults=50&playlistId=${pl}`); save('data-playlist-items', di);
  console.log('  playlist read-back has the video:', JSON.stringify(di).includes(videoId));
  const ll = await dataApi(token, 'GET', 'playlistItems?part=contentDetails&maxResults=50&playlistId=LL'); ll.items = ll.items.slice(0, 3); save('data-playlist-items-ll', ll, '; items trimmed to 3');
