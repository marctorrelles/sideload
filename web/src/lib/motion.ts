...
export function reveal(container: Element) {
  if (reduced()) return;
  Array.from(container.children).forEach((row, i) => (row as HTMLElement).style.setProperty('--i', String(Math.min(i, 12))));
...
export function countTo(el: HTMLElement, to: number, format: (v: number) => string, allowDown = false) {
  ...
  const run = String(Date.now()); el.dataset.run = run; // a newer call takes over mid-flight
...
export function onView(el: Element, cb: () => void) {
  ...
  const io = new IntersectionObserver(es => { if (es.some(e => e.isIntersecting)) { io.disconnect(); cb(); } }, { threshold: 0.4 });
  io.observe(el);
}
...
export function collapse(el: HTMLElement): Promise<void> {
  ...
  return new Promise(done => {
    const finish = () => { if (el.isConnected) el.remove(); done(); };
    requestAnimationFrame(() => { el.classList.add('collapsing'); el.style.height = '0'; });
...
export function crossfade(el: HTMLElement) {
  if (reduced()) return;
  el.classList.remove('xfade'); void el.offsetWidth; el.classList.add('xfade');
}
