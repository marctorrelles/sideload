...
export const SPOTIFY_SCOPES =
  'user-library-read playlist-read-private playlist-read-collaborative user-follow-read user-read-email';
...
export function toTrack(e: RawEntry): SpotifyTrack | null {
  ...
  return {
    ...
    isLocal: !!e.is_local || !!it.is_local,
    isEpisode: it.type === 'episode' || !it.artists,
    addedAt: e.added_at,
  };
}
...
export class Spotify {
  constructor(
    private t: Tokens,
    private onRefresh: (t: Tokens) => Promise<void>,
  ) {}
  private async get<T>(path: string): Promise<T> {
    if (this.t.access === DEMO_ACCESS) {
      // reviewer / demo library, nothing leaves the Worker
    ...
    if (this.t.expiresAt - Date.now() < 120_000) {
      ...
      this.t = {
        ...
        refresh: r.refresh_token ?? this.t.refresh,
    ...
    try {
      r = await fetch(path.startsWith('http') ? path : `${API}${path}`, {
        headers: { authorization: `Bearer ${this.t.access}` },
        signal: AbortSignal.timeout(30_000),
      });
    } catch (e) {
      throw new SpotifyError(429, 'timeout', `Spotify did not answer: ${String(e).slice(0, 120)}`, 10);
    } // D17: a hang is a retryable failure
    if (r.status === 429)
      throw new SpotifyError(429, 'rate_limited', 'Spotify rate limit', Number(r.headers.get('retry-after') ?? 5));
    if (r.status === 401) throw new SpotifyError(401, 'auth_expired');
    if (!r.ok)
      throw new SpotifyError(
        r.status,
        'http_error',
        `${path.startsWith('http') ? new URL(path).pathname : path.split('?')[0]}: ${(await r.text()).slice(0, 300)}`,
      );
    return r.json() as Promise<T>;
  }
  me() {
    return this.get<{ id: string; email?: string; display_name: string | null }>('/me');
  }
  playlists(url = '/me/playlists?limit=50') {
    return this.get<Page<RawPlaylist>>(url);
  }
  playlistItems(id: string, offset: number) {
    return this.get<Page<RawEntry>>(`/playlists/${id}/items?limit=50&offset=${offset}&additional_types=track,episode`);
  }
  savedTracks(offset: number) {
    return this.get<Page<RawEntry>>(`/me/tracks?limit=50&offset=${offset}`);
  }
  savedAlbums(url = '/me/albums?limit=50') {
    return this.get<
      Page<{
        album: {
          id: string;
