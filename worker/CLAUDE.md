...
| `spotify-demo.ts` | The built-in demo library (two playlists, five likes, an album, an artist; SVG data-URI covers). `Spotify.get` serves it instead of the API when the access token is `DEMO_ACCESS`; `/auth/spotify/start` hands out those tokens when the client id equals the `REVIEW_CODE` secret. Reviewers and demos only; the YouTube side stays real. |
...
| `innertube.ts` | One `InnerTube` class, three transports: `music()` anonymous on music.youtube.com, walking `MUSIC_PLAN` (`ANDROID_MUSIC`, `IOS_MUSIC`, Android, iOS, `WEB_REMIX`) with growing pauses (`retryDelayMs`, 250 ms × attempt; tests pass 0) and a `music_retry` log line per failed attempt; only the last failure surfaces as a throttle. From Cloudflare's egress the web client gets the abuse page or a 404 on every call and the app clients on a request here and there (measured 2026-09-03: Android 8/10, iOS 5/5, web 0/5), search songs/videos/albums/artists with parsers for both item shapes (`musicResponsiveListItemRenderer` and the Android `musicTwoColumnItemRenderer`, which carries no album and no artist links); the album's `OLAK5uy_` id comes from the search result's play overlay on both shapes, `albumPlaylistId()` (web-client browse, `microformat.urlCanonical`) only when it is missing; `tv()` `TVHTML5` on www.youtube.com with the bearer token (`addPlaylistItems`, `like`, `likePlaylist` = save album, `subscribe`); `data()` Data API v3 (`createPlaylist` = `playlists.insert`, `playlistVideoIds`/`likedVideoIds` = `playlistItems.list`, `LL` = liked videos). Errors: 401 → `AuthError`; 429/5xx/HTML-200/403-HTML-abuse-page/timeout → `ThrottleError`; Data API `quotaExceeded` → `ThrottleError` with `retryAfterMs` until midnight Pacific; other Data API 403 → `AuthError`. |
| `match.ts` | `stripFeat`, `buildQuery` (primary artist + title, no `&`), `cacheKey` (`artist|title`, NFKD, lowercase, punctuation collapsed), Dice-bigram `similarity`, `score` (title, artist, duration ×5, album ×0.3: the album term only breaks ties between same-title candidates), `pickBest` with the confidence gate (title ≥ 0.6, artist ≥ 0.5, duration Δ ≤ 15 s) and a plausibility gate for the review list's "closest" suggestion (title ≥ 0.5 and artist ≥ 0.3, or title ≥ 0.8; below that the row is `no_match` with search only, because "Use closest" on a 0.2-similar title adds a wrong song). `stripFeat` also drops Beatport's "Original Mix" suffix. Calibrated 2026-09-03 with `pnpm calibrate` (`worker/scripts/calibrate-match.ts`, live search against the author's accepted Python-tool picks, `CALIB_CACHE=file` replays offline): 100 tracks, 89 same id, 93 same song, confident 89 of 100 at 96.6% same-song precision; the misses are variant recordings (a "(Small Mix)"), never a different song. |
...
Logs are single-line JSON with `evt`: `throttle`, `verify`, `job_done`, `job_failed`, `tick_error`, `unhandled`, `job_created`, `telemetry_error` (a Mixpanel post failed; the job is unaffected), `music_retry` (one search attempt got the abuse page; the next client is tried). Job ids are logged as their first 6 characters.
...
- A TV-client OAuth token is accepted only with a `TVHTML5` context on www.youtube.com; `WEB_REMIX`/`WEB`/`ANDROID*` answer `400 INVALID_ARGUMENT`, and `TVHTML5` cannot create playlists (`Precondition check failed`) → Data API `playlists.insert` (D3). Anonymous search from the Worker: the app clients pass where `WEB_REMIX` is blocked, and the abuse page is per request (each fetch may leave from another egress IP), so retrying beats backing off; fixtures `android-search-*.json` were recorded from the edge with a scratch Worker under `wrangler dev --remote` (pruned of thumbnails, menus and player configs). Searches are anonymous, so throttling is per egress IP (D8): the abuse page (403 HTML "Sorry…") is a throttle, handled by backoff + the KV cache.
