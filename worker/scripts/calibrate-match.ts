...
const lookup = JSON.parse(readFileSync(`${home}/Library/Caches/spotify_to_ytmusic/lookup.json`, 'utf8')) as Record<
  string,
  string
>;
const fx = (f: string) =>
  JSON.parse(readFileSync(`worker/test/fixtures/${f}`, 'utf8')).items as Parameters<typeof toTrack>[0][];
const entries = [...fx('spotify-me-tracks.json'), ...fx('spotify-playlist-items.json')];
const cachePath = process.env.CALIB_CACHE;
const cache: Record<string, SearchSong[]> =
  cachePath && existsSync(cachePath) ? JSON.parse(readFileSync(cachePath, 'utf8')) : {};
const yt = new InnerTube(null);
const keysOf = (t: { artists: string[]; name: string }) => [
  `${t.artists.join(' ')} ${stripFeat(t.name)}`.replace(/ &/g, ''),
...
for (const e of entries) {
  const t = toTrack(e);
  if (!t || t.isLocal || t.isEpisode) continue;
  const expected = keysOf(t)
    .map((k) => lookup[k])
    .find(Boolean);
  if (!expected) {
    noKey++;
  ...
  let results = cache[q];
  if (!results) {
    results = cache[q] = await yt.searchSongs(q);
    await new Promise((r) => setTimeout(r, 1500));
  } // be polite to the anonymous endpoint
  const m = pickBest(t, results);
  n++;
  if (!m.best) {
    noResult++;
    disagree.push(`NONE  ${t.artists[0]} - ${t.name} (expected ${expected})`);
    continue;
  }
  ...
  const sameSong =
    same ||
    (!!alt && norm(alt.title) === norm(m.best.title) && norm(alt.artists.join(' ')) === norm(m.best.artists.join(' ')));
  if (same) agree++;
  if (sameSong) soft++;
  if (m.confident) {
    confident++;
    if (same) confAgree++;
    if (sameSong) confSoft++;
  }
  if (!sameSong) {
    disagree.push(
      `${m.confident ? 'CONF ' : 'low  '} ${t.artists[0]} - ${t.name} | ours ${m.best.videoId} "${m.best.title}" ${m.best.artists} ${m.score.toFixed(2)} | theirs ${expected}${alt ? ` "${alt.title}" ${alt.artists} (in results, rank ${results.indexOf(alt) + 1})` : ' (not in our top results)'}`,
    );
  }
    ...
    JSON.stringify(
      {
        ...
        noResult,
        agree,
        agreeRate: pct(agree, n),
        sameSongRate: pct(soft, n),
        confident,
        confAgree,
        confPrecision: pct(confAgree, confident),
        confSameSongPrecision: pct(confSoft, confident),
      },
      null,
      1,
    ),
