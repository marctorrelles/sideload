...
describe('google device flow', () => {
  it('requests a device code with the youtube scope', async () => {
    g()
      .intercept({ path: '/device/code', method: 'POST', body: (b) => String(b).includes('auth%2Fyoutube') })
      .reply(200, {
        device_code: 'd',
        user_code: 'ABCD-EFGH',
    ...
    expect(await deviceCode('cid')).toEqual({
      deviceCode: 'd',
      userCode: 'ABCD-EFGH',
  ...
  it('maps poll states', async () => {
    ...
    g()
      ...
      .reply(200, { access_token: 'a', refresh_token: 'r', expires_in: 3599 });
    const ok = await pollDevice('cid', 's', 'd');
    expect(ok.status).toBe('ok');
    if (ok.status === 'ok') expect(ok.refresh).toBe('r');
    g().intercept({ path: '/token', method: 'POST' }).reply(403, { error: 'access_denied' });
    expect(await pollDevice('cid', 's', 'd')).toEqual({ status: 'denied' });
  });
  it('refreshes', async () => {
    g()
      .intercept({ path: '/token', method: 'POST', body: (b) => String(b).includes('grant_type=refresh_token') })
      .reply(200, { access_token: 'a2', expires_in: 3599 });
