...
export interface StartPayload {
  ...
  tid?: string;
  spotify: Tokens;
  google: { access: string; refresh: string; expiresAt: number };
  selection: Selection;
}
...
interface JobRow {
  ...
  spotify_tokens: string | null;
  yt_tokens: string | null;
...
const REASON_TEXT: Record<string, string> = {
  ...
  low_confidence: 'needs a look',
  duplicate_match: 'same video as another song',
  unavailable: 'not available',
  local_file: 'local file',
};
class FatalError extends Error {
  constructor(public failure: JobFailure) {
    super(failure);
  }
}
const VIDEO_ID = /^[\w-]{11}$/;
...
export class JobDO extends DurableObject<Env> {
  ...
  private job(): JobRow | null {
    try {
      return (this.sql.exec('SELECT * FROM job LIMIT 1').toArray()[0] as JobRow | undefined) ?? null;
    } catch {
      return null;
    } // after deleteAll() a still-resident instance has no tables: treat as gone (→ 404)
  }
  ...
  async start(p: StartPayload): Promise<{ ok: true } | { ok: false; error: 'already_started' | 'too_large' }> {
    ...
    const now = Date.now();
    const [spot, yt] = await Promise.all([
      seal(this.env.TOKEN_SECRET, p.spotify),
      seal(this.env.TOKEN_SECRET, p.google),
    ]);
    this.ctx.storage.transactionSync(() => {
      this.sql.exec(
        'INSERT INTO job (id, tid, status, created_at, started_at, spotify_tokens, yt_tokens) VALUES (?,?,?,?,?,?,?)',
        p.id,
        p.tid ?? null,
        'running',
        now,
      ...
      const add = (id: string, kind: ItemKind, name: string, extra: Partial<ItemRow> = {}) =>
        this.sql.exec(
          'INSERT INTO item (id, ord, kind, spotify_id, name, artist, description, is_public, total) VALUES (?,?,?,?,?,?,?,?,?)',
          id,
          ord++,
          kind,
          extra.spotify_id ?? null,
          name,
          extra.artist ?? null,
      ...
      for (const pl of s.playlists)
        add(`pl:${pl.id}`, 'playlist', pl.name, {
          spotify_id: pl.id,
          description: pl.description,
          is_public: pl.isPublic ? 1 : 0,
          total: pl.trackCount,
        });
      for (const al of s.albums)
        add(`al:${al.id}`, 'album', al.name, { spotify_id: al.id, artist: al.artist, total: 1 });
  ...
  async disconnect(): Promise<void> {
    this.sql.exec('UPDATE job SET yt_tokens = NULL');
  }
  ...
  async view(): Promise<JobView | null> {
    ...
    for (const r of this.sql
      .exec('SELECT item_id, status, COUNT(*) AS n FROM track GROUP BY item_id, status')
      .toArray() as { item_id: string; status: string; n: number }[]) {
      const c = counts.get(r.item_id) ?? { moved: 0, review: 0, matched: 0, skipped: 0 };
      if (r.status in c) (c as any)[r.status] += r.n;
      counts.set(r.item_id, c);
    }
    ...
    const byReason = Object.fromEntries(
      (
        this.sql.exec("SELECT reason, COUNT(*) AS n FROM track WHERE status = 'review' GROUP BY reason").toArray() as {
          reason: string;
          n: number;
        }[]
      ).map((r) => [r.reason, r.n]),
    ...
    const totals = itemViews.reduce(
      (t, i) => ({
        ...t,
        tracks: t.tracks + i.total,
        moved: t.moved + i.moved,
        matched: t.matched + i.matched,
        review: t.review + i.review,
        skipped: t.skipped + (counts.get(i.id)?.skipped ?? 0),
      }),
      {
        tracks: 0,
        moved: 0,
        ...
        review: 0,
        skipped: 0,
        collapsed: byReason.duplicate_match ?? 0,
        writeFailed: byReason.write_failed ?? 0,
      },
    );
    ...
    return {
      ...
      totals,
      items: itemViews,
      review: this.reviewPage(0),
      reviewTotal: totals.review,
      startedAt: j.started_at,
      finishedAt: j.finished_at,
      ratePerMin,
      etaSeconds: ratePerMin && j.status === 'running' ? Math.round((remaining / ratePerMin) * 60) : null,
      ...
      covers: (
        this.sql
          .exec(
            "SELECT video_id FROM (SELECT id, video_id FROM track WHERE status IN ('matched', 'moved') AND video_id IS NOT NULL ORDER BY id DESC LIMIT 48) ORDER BY id ASC",
          )
          .toArray() as { video_id: string }[]
      ).map((r) => r.video_id),
      ytConnected: !!j.yt_tokens,
      searches: j.searches,
      cacheHits: j.cache_hits,
    };
  }
  async review(offset: number): Promise<ReviewItemView[]> {
    return this.reviewPage(Math.max(0, offset | 0));
  }
  private reviewPage(offset: number): ReviewItemView[] {
    const tracks = this.sql
      .exec(
        `SELECT t.*, i.name AS item_name, i.kind AS item_kind FROM track t JOIN item i ON i.id = t.item_id WHERE t.status = 'review' ORDER BY i.ord, t.pos LIMIT ? OFFSET ?`,
        REVIEW_PAGE,
        offset,
      )
      .toArray() as unknown as (TrackRow & { item_name: string; item_kind: ItemKind })[];
    const out: ReviewItemView[] = tracks.map((t) => ({
      id: t.id,
      kind: t.item_kind,
      title: t.name,
      artist: (JSON.parse(t.artists) as string[]).join(', '),
      itemName: t.item_name,
      reason: t.reason ?? 'no_match',
      suggestion: t.suggestion ? JSON.parse(t.suggestion) : null,
      collidesWith: t.collides_with,
      actionable: t.reason !== 'local_file',
    }));
    if (offset === 0)
      for (const i of this.sql
        .exec("SELECT * FROM item WHERE status = 'failed' ORDER BY ord")
        ...
        out.push({
          ...
          title: i.name,
          artist: i.artist ?? '',
          itemName: i.kind === 'album' ? 'Saved albums' : i.kind === 'artist' ? 'Followed artists' : 'Playlists',
          reason: (i.reason as ReviewReason | null) ?? 'no_match',
          suggestion: null,
          collidesWith: null,
          actionable: false,
        });
  ...
  async reportCsv(): Promise<string> {
    const rows = this.sql
      .exec(
        `SELECT i.name AS item_name, i.kind, t.name, t.artists, t.album, t.status, t.reason, t.video_id, t.suggestion, t.resolution FROM track t JOIN item i ON i.id = t.item_id ORDER BY i.ord, t.pos`,
      )
      .toArray() as any[];
    const entities = this.sql
      .exec("SELECT kind, name, artist, status, reason, yt_id FROM item WHERE kind IN ('album','artist') ORDER BY ord")
      .toArray() as any[];
    return toCsv([
      [
        'source',
      ...
      ...rows.map((r) => [
        ...
        'track',
        r.name,
        (JSON.parse(r.artists) as string[]).join('; '),
        r.album,
        r.status,
        r.reason,
        r.video_id,
        r.suggestion ? JSON.parse(r.suggestion).title : null,
        r.resolution,
      ]),
      ...entities.map((e) => [
        e.kind === 'album' ? 'Saved albums' : 'Followed artists',
        e.kind,
        e.name,
        e.artist,
        null,
        e.status === 'done' ? 'moved' : e.status,
        e.reason,
        e.yt_id,
        null,
        null,
  ...
  async resolve(
    trackId: number,
    a: ReviewAction,
  ): Promise<{ ok: true } | { ok: false; error: 'not_reviewable' | 'disconnected' | 'bad_video' | 'no_suggestion' }> {
    ...
    if (a.action === 'skip') {
      this.sql.exec("UPDATE track SET status = 'skipped', resolution = 'skipped' WHERE id = ?", trackId);
      return { ok: true };
    }
    const videoId =
      a.action === 'closest'
        ? t.suggestion
          ? (JSON.parse(t.suggestion) as { videoId: string }).videoId
          : null
        : a.videoId;
    ...
    if (item.status === 'done' || item.status === 'writing') {
      ...
      if (item.kind === 'liked') await yt.like(videoId);
      else await yt.addPlaylistItems(item.yt_id!, [videoId]);
      this.sql.exec(
        "UPDATE track SET status = 'moved', video_id = ?, resolution = ? WHERE id = ?",
        videoId,
        a.action,
        trackId,
      );
      ...
      this.sql.exec(
        "UPDATE track SET status = 'matched', video_id = ?, resolution = ? WHERE id = ?",
        videoId,
        a.action,
        trackId,
      ); // the normal write path will add it
    }
    return { ok: true };
  }
  async search(q: string): Promise<ManualSearchResult[] | null> {
    const job = this.job();
    if (!job?.yt_tokens) return null;
    const yt = await this.innertube(job);
    return (await yt.searchSongs(q.slice(0, 200)))
  ...
  async alarm(): Promise<void> {
    ...
    if (job.status === 'paused') {
      // resume() re-arms the work loop; here we only enforce the 24 h expiry
  ...
  private async step(sp: Spotify, yt: InnerTube): Promise<boolean> {
    const item = this.sql
      .exec("SELECT * FROM item WHERE status NOT IN ('done','failed') ORDER BY ord LIMIT 1")
      .toArray()[0] as ItemRow | undefined;
    if (!item) return true;
    if (item.kind === 'album' || item.kind === 'artist') {
      await this.moveEntity(item, yt);
      return false;
    }
    if (!item.fetched) {
      await this.fetchPage(item, sp);
      return false;
    }
    const pending = this.sql
      .exec("SELECT * FROM track WHERE item_id = ? AND status = 'pending' ORDER BY pos LIMIT ?", item.id, CONCURRENCY)
      .toArray() as unknown as TrackRow[];
    if (pending.length) {
      await this.matchBatch(pending, yt);
      return false;
    }
    if (item.status === 'verifying') {
      await (this.hasRedo(item) ? this.redrive(item, yt) : this.verifyItem(item, yt));
      return false;
    }
    await this.writeBatch(item, yt);
    return false;
  }
  private hasRedo(item: ItemRow): boolean {
    return (
      (this.sql.exec('SELECT COUNT(*) AS n FROM track WHERE item_id = ? AND redo > 0', item.id).one() as { n: number })
        .n > 0
    );
  }
  private async fetchPage(item: ItemRow, sp: Spotify): Promise<void> {
    if (item.fetch_offset === 0)
      await this.note(
        'read',
        item.kind === 'liked' ? 'Reading your liked songs' : `Reading "${item.name}"`,
        'from Spotify',
      );
    let page;
    try {
      page =
        item.kind === 'liked'
          ? await sp.savedTracks(item.fetch_offset)
          : await sp.playlistItems(item.spotify_id!, item.fetch_offset);
    } catch (e) {
      ...
      if (e instanceof SpotifyError && (e.status === 403 || e.status === 404)) {
        this.sql.exec(
          "UPDATE item SET status = 'failed', reason = 'not_accessible', fetched = 1 WHERE id = ?",
          item.id,
        );
        return;
      }
    ...
    this.ctx.storage.transactionSync(() => {
      page.items.forEach((raw, i) => {
        ...
        this.sql.exec(
          'INSERT INTO track (item_id, pos, spotify_id, name, artists, album, duration_ms, is_local, status, reason) VALUES (?,?,?,?,?,?,?,?,?,?)',
          item.id,
          item.fetch_offset + i,
          t.id,
          t.name,
          ...
          t.durationMs,
          t.isLocal ? 1 : 0,
          t.isLocal ? 'review' : 'pending',
          t.isLocal ? 'local_file' : null,
        );
      });
      const count = (this.sql.exec('SELECT COUNT(*) AS n FROM track WHERE item_id = ?', item.id).one() as { n: number })
        .n;
      ...
      this.sql.exec(
        'UPDATE item SET fetch_offset = ?, fetched = ?, status = ?, total = ? WHERE id = ?',
        item.fetch_offset + page.items.length,
        done ? 1 : 0,
        done ? 'matching' : 'fetching',
        done ? count : Math.max(item.total, count),
        item.id,
      );
    });
    if ((this.sql.exec('SELECT COUNT(*) AS n FROM track').one() as { n: number }).n > MAX_TRACKS)
      throw new FatalError('too_large');
  }
  private async matchBatch(tracks: TrackRow[], yt: InnerTube): Promise<void> {
      ...
      for (const r of results) {
        ...
        if (status === 'matched') {
          ...
          const other = this.sql
            .exec(
              "SELECT name FROM track WHERE item_id = ? AND video_id = ? AND id != ? AND COALESCE(spotify_id, '') != COALESCE(?, '') AND status IN ('matched', 'moved') LIMIT 1",
              r.itemId,
              r.videoId,
              r.id,
              r.spotifyId,
          ...
          if (other) {
            ...
            collidesWith = other.name;
            suggestion = JSON.stringify({
              videoId: r.videoId,
              title: `the same video as "${other.name}"`,
              artists: '',
            });
          }
        }
        this.sql.exec(
          'UPDATE track SET status = ?, video_id = ?, reason = ?, suggestion = ?, collides_with = ? WHERE id = ?',
          status,
          status === 'matched' ? r.videoId : null,
          reason ?? null,
          suggestion ?? null,
          collidesWith,
          r.id,
        ...
        this.sql.exec(
          'UPDATE job SET searches = searches + ?, cache_hits = cache_hits + ?',
          r.searched ? 1 : 0,
          r.hit ? 1 : 0,
        );
    ...
    for (const { r, status, reason } of events) {
      ...
      if (status === 'matched')
        await this.note('match', line, r.hit ? 'from the shared cache' : null, r.videoId ?? null);
  ...
  private async matchOne(
    ...
    if (m.best && m.plausible)
      return {
        ...base,
        status: 'review',
        reason: 'low_confidence',
        suggestion: JSON.stringify({
          videoId: m.best.videoId,
          title: m.best.title,
          artists: m.best.artists.join(', '),
        }),
        searched: true,
        hit: false,
      };
    return {
      ...base,
      status: 'review',
      reason: results.length && results.every((r) => r.unavailable) ? 'unavailable' : 'no_match',
      searched: true,
      hit: false,
    };
  }
  private async writeBatch(item: ItemRow, yt: InnerTube): Promise<void> {
    if (!item.yt_id) {
      ...
      const description =
        item.kind === 'liked' ? 'Your Spotify liked songs, moved with Sideload.' : (item.description ?? '');
      ...
      await this.note(
        'create',
        `Created "${title}"`,
        item.kind === 'liked' || !item.is_public ? 'private playlist' : 'public playlist',
      );
      return;
    }
    const batch = this.sql
      .exec(
        "SELECT id, video_id FROM track WHERE item_id = ? AND status = 'matched' ORDER BY pos ASC LIMIT ?",
        item.id,
        WRITE_BATCH,
      )
      .toArray() as { id: number; video_id: string }[];
    if (batch.length) {
      await yt.addPlaylistItems(
        item.yt_id,
      ...
      await this.note(
        'add',
        `Added ${batch.length} song${batch.length === 1 ? '' : 's'} to "${item.kind === 'liked' ? 'Liked Songs' : item.name}"`,
      );
      return;
    }
    if (item.kind === 'liked') {
      ...
      const likes = this.sql
        .exec(
          "SELECT id, video_id FROM track WHERE item_id = ? AND status = 'moved' AND liked = 0 ORDER BY pos DESC LIMIT ?",
          item.id,
          CONCURRENCY,
        )
        .toArray() as { id: number; video_id: string }[];
  ...
  private async verifyItem(item: ItemRow, yt: InnerTube): Promise<void> {
    const moved = this.sql
      .exec("SELECT id, video_id, liked FROM track WHERE item_id = ? AND status = 'moved'", item.id)
      .toArray() as { id: number; video_id: string; liked: number }[];
    ...
    console.log(
      JSON.stringify({
        ...
        pass: passes,
        moved: moved.length,
        missingAdds: missingAdds.length,
        missingLikes: missingLikes.length,
      }),
    );
    const missing = missingAdds.length + missingLikes.length;
    await this.note(
      'verify',
      `Checked "${item.kind === 'liked' ? 'Liked Songs' : item.name}" on YouTube`,
      missing ? `${missing} missing, adding again` : `all ${moved.length} there`,
    );
    this.ctx.storage.transactionSync(() => {
      ...
      if (passes <= MAX_VERIFY_PASSES) {
        // mark for re-drive; the next steps write them and we verify again
        if (missingAdds.length)
          this.sql.exec(`UPDATE track SET redo = 1 WHERE id IN (${ids(missingAdds)})`, ...missingAdds.map((t) => t.id));
        if (missingLikes.length)
          this.sql.exec(
            `UPDATE track SET redo = 2 WHERE id IN (${ids(missingLikes)})`,
            ...missingLikes.map((t) => t.id),
          );
        return;
      }
      ...
      for (const t of missingAdds)
        this.sql.exec(
          "UPDATE track SET status = 'review', reason = 'write_failed', suggestion = ? WHERE id = ?",
          JSON.stringify({ videoId: t.video_id, title: 'the same video, added again', artists: '' }),
          t.id,
        );
      finishItem();
    });
  ...
  private async redrive(item: ItemRow, yt: InnerTube): Promise<void> {
    const adds = this.sql
      .exec(
        'SELECT id, video_id FROM track WHERE item_id = ? AND redo = 1 ORDER BY pos ASC LIMIT ?',
        item.id,
        WRITE_BATCH,
      )
      .toArray() as { id: number; video_id: string }[];
    ...
    const likes = this.sql
      .exec(
        'SELECT id, video_id FROM track WHERE item_id = ? AND redo = 2 ORDER BY pos DESC LIMIT ?',
        item.id,
        CONCURRENCY,
      )
      .toArray() as { id: number; video_id: string }[];
  ...
  private async moveEntity(item: ItemRow, yt: InnerTube): Promise<void> {
    ...
    const fail = async () => {
      this.sql.exec("UPDATE item SET status = 'failed', reason = 'no_match' WHERE id = ?", item.id);
      await this.note(
        'review',
        item.kind === 'artist' ? item.name : `${item.artist ?? ''} · ${item.name}`,
    ...
    if (item.kind === 'artist') {
      ...
      const best = (await yt.searchAlbums(`${item.artist ?? ''} ${item.name}`.trim())).find(
        (h) =>
          similarity(h.title, item.name) >= 0.7 &&
          (!item.artist || similarity(h.artists.join(' '), item.artist) >= 0.5),
      );
  ...
  private async finish(): Promise<void> {
    ...
    this.sql.exec(
      "UPDATE job SET status = 'done', finished_at = ?, spotify_tokens = NULL, throttled_until = NULL",
      Date.now(),
    );
    await this.ctx.storage.setAlarm(Date.now() + REVIEW_GRACE_MS);
    this.ctx.waitUntil(
      this.env.STATS.get(this.env.STATS.idFromName('global'))
        .add({ moved: v.totals.moved, total: v.totals.tracks, seconds: Math.round((Date.now() - v.startedAt) / 1000) })
        .catch(() => {}),
    );
    ...
    const summary = {
      ...
      seconds: Math.round((Date.now() - v.startedAt) / 1000),
    };
  ...
  private async fail(failure: JobFailure): Promise<void> {
    this.sql.exec(
      "UPDATE job SET status = 'failed', failure = ?, finished_at = ?, spotify_tokens = NULL, yt_tokens = NULL",
      failure,
      Date.now(),
    );
  ...
  private async handleError(e: unknown, job: JobRow): Promise<void> {
    ...
    if (e instanceof ThrottleError || (e instanceof SpotifyError && e.status === 429)) {
      const wait =
        e instanceof SpotifyError
          ? e.retryAfter * 1000
          : (e.retryAfterMs ?? Math.min(5_000 * 2 ** (attempt - 1), 600_000)); // retryAfterMs: Data API daily quota → next midnight Pacific
      this.sql.exec('UPDATE job SET attempt = ?, throttled_until = ?', attempt, Date.now() + wait);
      console.log(
        JSON.stringify({ evt: 'throttle', job: job.id.slice(0, 6), attempt, wait, err: String(e).slice(0, 200) }),
      );
    ...
    if (e instanceof AuthError || e instanceof GoogleError || (e instanceof SpotifyError && e.status === 401))
      return this.fail('auth_expired');
    if (e instanceof FatalError) return this.fail(e.failure);
    Sentry.captureException(e);
    console.error(
      JSON.stringify({ evt: 'tick_error', job: job.id.slice(0, 6), attempt, err: String(e).slice(0, 300) }),
    );
  ...
  private async innertube(job: JobRow): Promise<InnerTube> {
    const g = await open<{ access: string; refresh: string; expiresAt: number }>(
  ...
  private async spotify(job: JobRow): Promise<Spotify> {
    ...
    return new Spotify(t, async (nt) => {
      this.sql.exec('UPDATE job SET spotify_tokens = ?', await seal(this.env.TOKEN_SECRET, nt));
    });
  }
}
