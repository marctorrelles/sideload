...
export async function seal(secret: string, data: unknown): Promise<string> {
  ...
  const ct = new Uint8Array(await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, await key(secret), enc.encode(JSON.stringify(data))));
...
export function randomId(): string {
  ...
  for (const b of bytes) {
    ...
    while (bits >= 5) { out += ALPHABET[(value >>> (bits - 5)) & 31]; bits -= 5; value &= (1 << bits) - 1; }
  }
  if (bits > 0) out += ALPHABET[(value << (5 - bits)) & 31];
  return out;
}
export function pkceVerifier(): string { return b64url(crypto.getRandomValues(new Uint8Array(48))); }
