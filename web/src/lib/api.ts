...
async function req<T>(path: string, init: RequestInit = {}): Promise<T> {
  const r = await fetch(path, { ...init, headers: { 'content-type': 'application/json', ...(init.headers ?? {}) }, credentials: 'same-origin' });
  if (!r.ok) { const j = await r.json().catch(() => ({})) as { error?: string; message?: string }; throw new ApiError(r.status, j.error ?? 'http', j.message ?? `Request failed (${r.status})`); }
...
export const api = {
  ...
  googleStart: () => post<{ userCode: string; verificationUrl: string; interval: number; expiresIn: number }>('/auth/google/start'),
