...
export default function Connect() {
  ...
  const [clientId, setClientId] = useState(() => { try { return localStorage.getItem('sl_client_id') ?? ''; } catch { return ''; } });
  ...
  async function startSpotify(e: Event) {
    ...
    try {
      try { localStorage.setItem('sl_client_id', id); } catch {}
      const { url } = await api.spotifyStart(id);
      location.href = url;
    } catch (x) { setErr(x instanceof ApiError ? x.message : 'Could not start the Spotify sign-in.'); }
  }
  async function startGoogle() {
    setDevState('code');
    let d: Dev;
    try { d = await api.googleStart(); } catch { setDevState('error'); return; }
    setDev(d);
