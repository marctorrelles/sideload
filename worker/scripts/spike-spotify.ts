...
if (!/^[a-f0-9]{32}$/.test(clientId)) {
  console.error(
    'Set SPOTIFY_CLIENT_ID (32 hex chars) in .dev.vars or the environment. Create the app at https://developer.spotify.com/dashboard with redirect URI http://127.0.0.1:8787/auth/spotify/callback',
  );
  process.exit(1);
}
...
createServer(async (req, res) => {
  ...
  if (q.get('error') || !q.get('code') || q.get('state') !== 'spike') {
    console.error('authorize returned:', Object.fromEntries(q));
    process.exit(1);
  }
  ...
  if (!tr.ok || !tok.access_token) {
    console.error('token exchange failed:', tr.status, JSON.stringify(tok));
    process.exit(1);
  }
  ...
  const get = async (p: string) => {
    ...
    if (!r.ok) {
      ...
      if (r.status === 403) {
        console.error(
          '  → not available with this app/account; note it in docs/design/handoff.md and keep the synthetic fixture',
        );
        return null;
      }
      process.exit(1);
  ...
  const ownerId = (id: string) => {
    if (!owners.has(id)) owners.set(id, `owner_${owners.size - 1}`);
    return owners.get(id)!;
  };
  const redact = (o: any, key = ''): any =>
    Array.isArray(o)
      ? o.map((x) => redact(x))
      : o && typeof o === 'object'
        ? Object.fromEntries(
            Object.entries(o).map(([k, v]) => [
              k,
              (['email', 'display_name', 'country', 'uri', 'href', 'snapshot_id'].includes(k) ||
                k === 'external_urls') &&
              v
                ? k === 'external_urls'
                  ? { spotify: 'REDACTED' }
                  : 'REDACTED'
                : k === 'id' && typeof v === 'string' && owners.has(v)
                  ? owners.get(v)
                  : k === 'owner' && v && typeof v === 'object'
                    ? { ...redact(v), id: ownerId((v as any).id) }
                    : k === 'added_by' && v
                      ? { id: 'REDACTED' }
                      : redact(v, k),
            ]),
          )
        : typeof o === 'string' && o.includes(me.id)
          ? o.replaceAll(me.id, 'me')
          : o;
  const save = (n: string, j: unknown) =>
    j &&
    writeFileSync(
      `worker/test/fixtures/${n}.json`,
      JSON.stringify(
        {
          _recorded: `${new Date().toISOString().slice(0, 10)} with worker/scripts/spike-spotify.ts; identity fields redacted, owner ids replaced`,
          ...redact(j),
        },
        null,
        1,
  ...
  const pl = await get('/me/playlists?limit=50');
  save('spotify-me-playlists', pl);
  console.log(
    'first playlist keys:',
    Object.keys(pl.items[0]),
    'items.total =',
    pl.items[0].items?.total,
    'tracks =',
    pl.items[0].tracks,
  );
  const own =
    pl.items.find((x: any) => x.owner?.id === me.id && (x.items?.total ?? x.tracks?.total ?? 0) > 10) ??
    pl.items.find((x: any) => x.owner?.id === me.id) ??
    pl.items[0];
  const other = pl.items.find((x: any) => x.owner?.id !== me.id && x.owner?.id !== 'spotify');
  if (other) {
    console.log('probing a followed playlist owned by someone else (expected 403 in Development Mode):');
    await get(`/playlists/${other.id}/items?limit=1`);
  }
