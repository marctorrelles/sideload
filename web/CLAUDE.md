...
| `src/pages/select.astro` + `src/islands/Select.tsx` | Step 02. Tabs, tri-state select-all, shift-click ranges, sort, every row rendered (no pagination, no virtualisation; 500-playlist cap), summary panel → sticky bar on mobile. Playlists owned by other users render locked (a Development Mode Spotify app cannot read them). |
