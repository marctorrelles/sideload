...
export class ThrottleError extends Error {
  ...
  constructor(public status: number, detail = '', public retryAfterMs?: number) { super(`youtube throttled (${status})${detail ? `: ${detail}` : ''}`); }
}
export class AuthError extends Error { constructor(detail = '') { super(`youtube auth rejected${detail ? `: ${detail}` : ''}`); } }
...
export function msUntilQuotaReset(now = Date.now()): number {
  const parts = Object.fromEntries(new Intl.DateTimeFormat('en-US', { timeZone: 'America/Los_Angeles', hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' }).formatToParts(new Date(now)).map(p => [p.type, p.value]));
...
export function parseSong(r: J): SearchSong | null {
  ...
  if (!artists.length) artists = (runs.map(x => String(x.text)).join('').split(' • ')[0] ?? '').split(/, | & /).filter(Boolean); // some results carry no artist links; text fallback
...
export class InnerTube {
  ...
  private async post(url: string, headers: Record<string, string>, body: object): Promise<J> {
    ...
    if (r.status === 403) throw text.trimStart().startsWith('{') ? new AuthError('403') : new ThrottleError(403, 'abuse page'); // measured: Google's "Sorry…" HTML page = throttle, not a permission problem
    let j: J;
    try { j = JSON.parse(text); } catch { throw new ThrottleError(0, 'non-JSON 200'); } // measured: throttled responses can be HTML with status 200
  ...
  async tv(endpoint: string, body: object): Promise<J> {
    ...
    return this.post(`${TV}${endpoint}?prettyPrint=false`, { 'content-type': 'application/json', 'user-agent': TV_UA, authorization: `Bearer ${this.token}` },
      { ...body, context: { client: { clientName: 'TVHTML5', clientVersion: '7.20250120.19.00', hl: 'en', gl: 'US' }, user: {} } });
  }
  /** YouTube Data API v3 (quota: list = 1 unit, insert = 50). */
  async data(method: 'GET' | 'POST', path: string, body?: object): Promise<J> {
    if (!this.token) throw new AuthError('no token');
    let r: Response;
    try { r = await fetch(`${DATA}${path}`, { method, headers: { authorization: `Bearer ${this.token}`, 'content-type': 'application/json' }, body: body ? JSON.stringify(body) : undefined, signal: AbortSignal.timeout(this.timeoutMs) }); }
    catch (e) { throw new ThrottleError(-1, String(e).slice(0, 120)); }
    const j: J = await r.json().catch(() => ({}));
    if (r.ok) return j;
    const reason: string = j?.error?.errors?.[0]?.reason ?? '';
    if (r.status === 401) throw new AuthError();
    if (r.status === 403 && /quotaExceeded|dailyLimit/i.test(reason)) throw new ThrottleError(403, reason, msUntilQuotaReset()); // project-wide daily quota
    ...
    throw new Error(`data api ${path.split('?')[0]}: ${r.status} ${reason || String(j?.error?.message ?? '').slice(0, 120)}`);
  }

  // ---- search (anonymous)
  private async search(q: string, params: string): Promise<J[]> { return shelves(await this.music('search', { query: q, params })); }
  ...
  async createPlaylist(title: string, description: string, privacy: 'PRIVATE' | 'PUBLIC' | 'UNLISTED'): Promise<string> {
    const j = await this.data('POST', 'playlists?part=snippet,status', { snippet: { title: title.slice(0, 150) || 'Untitled', description: description.slice(0, 1000) }, status: { privacyStatus: privacy.toLowerCase() } });
  ...
  async addPlaylistItems(playlistId: string, videoIds: string[]): Promise<void> {
    const j = await this.tv('browse/edit_playlist', { playlistId, actions: videoIds.map(v => ({ action: 'ACTION_ADD_VIDEO', addedVideoId: v })) });
    if (j?.status && j.status !== 'STATUS_SUCCEEDED') throw new Error(`edit_playlist: ${j.status}`);
  }
  like(videoId: string) { return this.tv('like/like', { target: { videoId } }); }
  /** Saving an album = liking its `OLAK5uy_` playlist. */
  likePlaylist(playlistId: string) { return this.tv('like/like', { target: { playlistId } }); }
  subscribe(channelId: string) { return this.tv('subscription/subscribe', { channelIds: [channelId] }); }
  ...
  async playlistVideoIds(playlistId: string): Promise<Set<string>> {
    ...
    for (let pageToken = '', guard = 0; guard < 1000; guard++) { // 1000 pages ≫ MAX_TRACKS
      const j = await this.data('GET', `playlistItems?part=contentDetails&maxResults=50&playlistId=${encodeURIComponent(playlistId)}${pageToken ? `&pageToken=${encodeURIComponent(pageToken)}` : ''}`);
      for (const it of j?.items ?? []) { const v = it?.contentDetails?.videoId; if (typeof v === 'string') ids.add(v); }
      pageToken = typeof j?.nextPageToken === 'string' ? j.nextPageToken : '';
      if (!pageToken) break;
    }
    return ids;
  }
  likedVideoIds() { return this.playlistVideoIds('LL'); }
}
