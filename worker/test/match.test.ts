...
const song = (o: Partial<import('../src/innertube').SearchSong>) => ({
  videoId: 'v',
  title: 'Xtal',
  artists: ['Aphex Twin'],
  album: 'Selected Ambient Works 85-92',
...
describe('match', () => {
  ...
  it('cache key is accent/punctuation/case insensitive', () => {
    expect(cacheKey({ name: 'Déjà Vu!', artists: ['Beyoncé'] })).toBe(
      cacheKey({ name: 'deja vu', artists: ['BEYONCE'] }),
    );
  });
  ...
  it('is confident when the candidate names one of several Spotify artists', () => {
    const m = pickBest({ ...t, name: 'The Rite Place - Crazy P Remix', artists: ['Blakkat', 'Aswan', 'Crazy P'] }, [
      song({ title: 'The Rite Place (Crazy P Remix) (feat. Aswan)', artists: ['Blakkat'] }),
    ]);
    expect(m.confident).toBe(true);
    expect(
      pickBest({ ...t, name: 'Con Mi Sombra', artists: ['S-Tone Inc'] }, [
        song({ title: 'Con Mi Sombra', artists: ['Hôtel Costes', 'STone Inc'] }),
      ]).confident,
    ).toBe(true);
  });
  it('similarity', () => {
    expect(similarity('Xtal', 'Xtal')).toBe(1);
    expect(similarity('Xtal', 'zzzz')).toBe(0);
    expect(similarity('Untitled', 'Untitled (Selected Ambient Works)')).toBeGreaterThan(0.3);
  }); // Dice bigrams ≈ 0.36 here; pickBest's 0.6 title gate still sends it to review
  it('prefers the song over the video with the same title', () => {
    ...
    expect(m.best!.videoId).toBe('song');
    expect(m.confident).toBe(true);
  });
  it('prefers the exact title on a single over a variant title on the right album', () => {
    ...
    const m = pickBest(t, [
      song({ videoId: 'variant', title: 'Xtal (Gym Cover)' }),
      song({ videoId: 'exact', album: 'Xtal' }),
    ]);
