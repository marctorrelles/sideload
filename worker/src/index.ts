...
app.onError((e, c) => {
  if (e instanceof HttpError)
    return withSecurityHeaders(c.json({ error: e.code, message: e.message }, e.status), c.req.path);
  if (e instanceof SpotifyError) {
    // Spotify's own words reach the user; the endpoint reaches the log
    const premium = /premium/i.test(e.message);
    console.error(
      JSON.stringify({ evt: 'spotify_error', path: c.req.path, status: e.status, err: e.message.slice(0, 300) }),
    );
    const message = premium
      ...
      : e.status === 401
        ? 'Your Spotify sign-in expired. Connect it again.'
        : `Spotify answered ${e.status} on ${e.message.split(':')[0]}. Try again in a minute.`;
    return withSecurityHeaders(
      c.json(
        { error: premium ? 'spotify_premium_required' : `spotify_${e.code}`, message },
        e.status === 401 ? 401 : e.status === 403 ? 403 : 502,
      ),
      c.req.path,
    );
  }
  ...
  return withSecurityHeaders(
    c.json({ error: 'internal', message: 'Something broke on our side. Try again in a minute.' }, 500),
    c.req.path,
  );
});
...
const sessionView = (s: Session | null): SessionView => ({
  spotify: s?.spotify
    ? {
        displayName: s.spotify.displayName,
        email: s.spotify.email,
        clientId: s.spotify.clientId,
        counts: s.spotify.counts,
      }
    : null,
  destination: s?.google ? { provider: 'ytmusic', account: s.google.account ?? null } : null,
});
const jobStub = (c: { env: Env }, id: string) => {
  if (!ID_RE.test(id)) throw new HttpError(404, 'not_found');
  return c.env.JOB.get(c.env.JOB.idFromName(id));
};
...
app.post('/auth/spotify/start', rateLimit('RL_AUTH'), async (c) => {
  ...
  if (!CLIENT_ID_RE.test(clientId))
    throw new HttpError(
      400,
      'bad_client_id',
      'A Spotify Client ID is 32 hex characters. Copy it from your app in the Spotify dashboard.',
    );
  if (c.env.REVIEW_CODE && clientId === c.env.REVIEW_CODE) {
    // the built-in demo library: no Spotify round trip, straight to "connected"
    const s = (await readSession(c, c.env)) ?? {};
    await writeSession(c, c.env, {
      ...s,
      tid: s.tid ?? randomId(),
      spotify: {
        ...demoTokens(clientId),
        userId: DEMO_USER.id,
        email: null,
        displayName: DEMO_USER.display_name,
        counts: DEMO_COUNTS,
      },
    });
  ...
  await writeTransient(c, c.env, {
    ...((await readTransient(c, c.env)) ?? {}),
    spotify: { state, verifier, clientId },
  });
  return c.json({
    url: authorizeUrl({ clientId, redirectUri: redirectUri(c.env), state, challenge: await pkceChallenge(verifier) }),
  });
});
app.get('/auth/spotify/callback', async (c) => {
  ...
  try {
    ...
    const [me, pl, liked] = await Promise.all([
      sp.me(),
      sp.playlists('/me/playlists?limit=1'),
      sp.savedTracks(0).catch(() => ({ total: 0 })),
    ]);
    const s = (await readSession(c, c.env)) ?? {};
    ...
    await writeSession(c, c.env, {
      ...
      spotify: {
        ...
        counts: { playlists: pl.total, liked: liked.total },
      },
    });
    ...
    return back(
      e instanceof SpotifyError ? (/premium/i.test(e.message) ? 'premium_required' : e.code) : 'token_exchange_failed',
    );
  }
});
app.post('/auth/spotify/logout', async (c) => {
  const s = (await readSession(c, c.env)) ?? {};
  delete s.spotify;
  await writeSession(c, c.env, s);
...
app.post('/auth/google/start', rateLimit('RL_AUTH'), async (c) => {
  ...
  await writeTransient(c, c.env, {
    ...((await readTransient(c, c.env)) ?? {}),
    google: { deviceCode: d.deviceCode, expiresAt: Date.now() + d.expiresIn * 1000 },
  });
  return c.json({
    userCode: d.userCode,
...
app.post('/auth/google/poll', rateLimit('RL_READ'), async (c) => {
  ...
  await writeSession(c, c.env, {
    ...s,
    google: { access: r.access, refresh: r.refresh, expiresAt: r.expiresAt, account: await channelInfo(r.access) },
  });
  c.executionCtx.waitUntil(track(c.env, 'youtube_connected', s.tid));
  delete tr.google;
  await writeTransient(c, c.env, tr);
  return c.json({ status: 'connected' });
});
app.post('/auth/google/logout', async (c) => {
  const s = (await readSession(c, c.env)) ?? {};
  delete s.google;
  await writeSession(c, c.env, s);
  return c.json({ ok: true });
...
app.get('/api/library', rateLimit('RL_READ'), async (c) => {
  ...
  const sp = new Spotify(s.spotify, async (t) => {
    await writeSession(c, c.env, { ...s, spotify: { ...s.spotify!, ...t } });
  });
  ...
  for (let url: string | null = '/me/playlists?limit=50'; url;) {
    ...
    for (const x of p.items)
      playlists.push({
        ...
        owner: x.owner.display_name ?? x.owner.id,
        ownedByUser: x.owner.id === s.spotify.userId,
        isAlgorithmic: x.owner.id === 'spotify',
        collaborative: x.collaborative,
        isPublic: x.public,
        trackCount: x.items?.total ?? x.tracks?.total ?? null,
        image: x.images?.[0]?.url ?? null,
      });
    url = p.next;
  }
  const albums: Library['albums'] = [];
  for (let url: string | null = '/me/albums?limit=50'; url;) {
    const p = await sp.savedAlbums(url);
    for (const { album } of p.items)
      albums.push({
        id: album.id,
        name: album.name,
  ...
  for (let after: string | undefined; ;) {
    const p = await sp.followedArtists(after);
    for (const a of p.artists.items) artists.push({ id: a.id, name: a.name, image: a.images?.[0]?.url ?? null });
    if (!p.artists.cursors.after || !p.artists.items.length) break;
    after = p.artists.cursors.after;
  }
  const lib: Library = { likedCount: s.spotify.counts.liked, playlists, albums, artists };
  c.executionCtx.waitUntil(
    track(c.env, 'library_loaded', s.tid, {
      playlists: playlists.length,
      locked: playlists.filter((p) => !p.ownedByUser).length,
      albums: albums.length,
      artists: artists.length,
      liked: lib.likedCount,
    }),
  );
  return c.json(lib);
});
...
app.post('/api/jobs', rateLimit('RL_JOB_CREATE'), async (c) => {
  ...
  const id = randomId();
  const r = await c.env.JOB.get(c.env.JOB.idFromName(id)).start({
    id,
    tid: s.tid,
    spotify: {
      clientId: s.spotify.clientId,
      access: s.spotify.access,
      refresh: s.spotify.refresh,
      expiresAt: s.spotify.expiresAt,
    },
    google: s.google,
    selection,
  });
  if (!r.ok)
    throw r.error === 'too_large'
      ? new HttpError(413, 'too_large', 'That is more than 25,000 songs. Split it into two transfers.')
      : new HttpError(409, r.error);
  clearSession(c);
  console.log(
    JSON.stringify({
      evt: 'job_created',
      job: id.slice(0, 6),
      playlists: selection.playlists.length,
      liked: selection.liked,
      albums: selection.albums.length,
      artists: selection.artists.length,
    }),
  );
  c.executionCtx.waitUntil(
    track(c.env, 'job_created', s.tid, {
      job: id.slice(0, 6),
      playlists: selection.playlists.length,
      liked: selection.liked,
      albums: selection.albums.length,
...
app.get('/api/jobs/:id', rateLimit('RL_READ'), async (c) => {
  const v = await jobStub(c, c.req.param('id')).view();
  if (!v) throw new HttpError(404, 'not_found');
  return c.json(v);
});
app.get('/api/jobs/:id/review', rateLimit('RL_READ'), async (c) =>
  c.json(await jobStub(c, c.req.param('id')).review(Number(c.req.query('offset') ?? 0))),
);
app.post('/api/jobs/:id/pause', rateLimit('RL_READ'), async (c) => {
  await jobStub(c, c.req.param('id')).pause();
  return c.json({ ok: true });
});
app.post('/api/jobs/:id/resume', rateLimit('RL_READ'), async (c) => {
  await jobStub(c, c.req.param('id')).resume();
  return c.json({ ok: true });
});
app.post('/api/jobs/:id/disconnect', rateLimit('RL_READ'), async (c) => {
  await jobStub(c, c.req.param('id')).disconnect();
  return c.json({ ok: true });
});
...
app.get(
  '/api/jobs/:id/search',
  rateLimit('RL_SEARCH', (c) => c.req.param('id') ?? ''),
  async (c) => {
    ...
    if (!r)
      throw new HttpError(
        409,
        'disconnected',
        'The YouTube Music connection was dropped. Start another transfer to search again.',
      );
    return c.json(r);
  },
);
app.get('/api/jobs/:id/report.csv', rateLimit('RL_READ'), async (c) => {
  ...
  return new Response(await stub.reportCsv(), {
    headers: {
      'content-type': 'text/csv; charset=utf-8',
      'content-disposition': `attachment; filename="sideload-report-${c.req.param('id').slice(0, 8)}.csv"`,
...
app.get('/api/stats', async (c) => {
  ...
  return c.json(await c.env.STATS.get(c.env.STATS.idFromName('global')).get());
});

// ---- app shell for /t/:id (static assets do not know the id)
app.get('/t/:id', async (c) => {
  if (!ID_RE.test(c.req.param('id'))) {
    const nf = await c.env.ASSETS.fetch(new Request(new URL('/404', c.req.url)));
